...
- **Compact Payloads & Disk-Offloaded Reports:** Subagents write exhaustive test transcripts, diff breakdowns, and file logs directly to disk, returning strictly bounded structured YAML payloads ($\le 250$ tokens) to caller prompts to eliminate quadratic $O(N^2)$ token re-transmission.
...
3. Invokes ephemeral `stage-runner` to coordinate the single stage lifecycle:
   - `stage-architect` writes `NN-slug.md` (contract) and `NN-slug.detail.md` (spec) on disk, grounded by prior stage report summaries for Stage $N > 1$.
   - In non-yolo mode, `stage-runner` displays and verifies the stage plan with the user.
   - `implementer` writes code, adds long-term tests to project test directory, isolates scratch checks in `scratchpad/`, writes full evidence to disk, and returns a compact structured payload ($\le 250$ tokens).
   - `verifier` independently inspects diff and executes tests with `run_command`.
   - On `VERDICT: FAIL`, `implementer` self-heals (max 2 cycles).
   - On `VERDICT: PASS`, commits stage (`<feature>-stage-<num>: <title>`) and finalizes `NN-slug.report.md`.
4. `stage-runner` returns a compact stage summary ($\le 1000$ tokens) to the Root Orchestrator and terminates. Root marks stage `done` in `STATE.md` and stops.
...
- Runs an isolated ephemeral `stage-runner` for each stage.
...
stage cleanup
```
Reviews decisions with minimum token overhead and executes cleanup:
