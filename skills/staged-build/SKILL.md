...
description: >-
  ...
  review diffs, validate behavior black-box, debug failures, or run full plans unattended with yolo mode.
...
A multi-agent development pipeline in which complex software engineering goals are broken into independently shippable stages. Every stage is rigorously specified, verified against the codebase before coding begins, implemented, peer-reviewed, independently black-box validated, and debugged if necessary.
...
| `stage next [--new-branch]` | Executes exactly **one** pending stage through the full pipeline: specify $\to$ check $\to$ implement $\to$ review $\to$ validate. | Stops after validating stage and producing report. Never auto-advances. |
| `stage yolo ["<goal>"]` | Runs the entire plan end-to-end unattended. Resolves minor decisions, logs to `DECISIONS.md`, and commits each stage. | Stops only on major architectural decisions, unrecoverable failures, or when all stages finish. |
...
2. **Context Isolation:** Subagents start blank and share no conversational context. All file dependencies and prior findings must be passed verbatim in the delegation prompt.
3. **Validator Independence:** The `validator` receives **ONLY the stage acceptance contract (`NN-slug.md`)** and nothing else. Never pass implementation specs (`.detail.md`), implementer reports, diffs, reviewer verdicts, or decision logs.
4. **Pre-Implementation Verification:** No stage is implemented until `plan-checker` falsifies the spec against the live codebase (both whole-stage and per-`[large]` step).
5. **Single Branch per Feature:** One Git branch `staged-build/<feature>` for all stages of a feature. Recorded on the `Branch:` line of `specs/<feature>/STATE.md`.
...
specs/<feature>/
  ...
  stages/
    ...
    NN-slug.report.md        # Stage completion report with validation evidence
```
...
- `TypeName`: The role name (e.g. `plan-architect`, `stage-architect`, `plan-checker`, `implementer`, `reviewer`, `validator`, `debugger`).
- `Role`: Descriptive role title (e.g. `Staged Build Stage Architect`).
- `Model`: Resolved model tier (`"pro"`, `"flash"`, `"flash_lite"`, `"inherit"`).
- `Workspace`: `"inherit"`.
...
1. **Proposal Pass:** Invoke `plan-architect` on model `pro` with the goal and instruction: `"PROPOSAL PASS: Write no files to disk. Read codebase, assess feasibility, isolate unknowns into Stage 01, and propose numbered stages and specific questions."`
...
flowchart TD
    ...
    D --> E[Delegate to plan-checker\nCheck stage plan & large steps]
    E -- FAIL (max 2) --> D_Rev[Revise plan with stage-architect] --> E
    E -- PASS --> F[Record Stage Base Commit\nDelegate to implementer]
    F --> G[Delegate to reviewer\nReview diff vs NN-slug.md]
    G -- FAIL (max 2) --> H[Debug with debugger] --> G
    G -- PASS --> I[Delegate to validator\nBlack-box with ONLY NN-slug.md]
    I -- FAIL (max 2) --> J[Debug with debugger] --> I
    I -- PASS --> K[Write NN-slug.report.md\nMark done in STATE.md\nSuggest commit & STOP]
```
...
3. **Specify Stage:** Invoke `stage-architect` (model `pro`) with `SPEC.md` and the stage's row. It creates `NN-slug.md` (contract) and `NN-slug.detail.md` (implementation spec).
   - If it splits the stage or reports a conflict, stop and report.
4. **Check Plan:**
   - Invoke `plan-checker` (model `pro`) in **stage plan** mode with `NN-slug.md` and `NN-slug.detail.md`.
   - For every step marked `[large]`, invoke `plan-checker` in **step plan** mode (one invocation per large step).
   - **On FAIL:** Invoke `stage-architect` for revision pass (max 2 cycles). If still failing, halt.
5. **Implement:**
...
6. **Review:**
   ...
   - Invoke `reviewer` (model `pro`) with `NN-slug.md` and the diff.
   - **On FAIL:** Invoke `debugger` (model `pro`) with findings $\to$ re-review (max 2 cycles).
7. **Validate:**
   - Invoke `validator` (model `flash`) with **ONLY `NN-slug.md`**.
   - **On FAIL:** Invoke `debugger` with validator findings $\to$ re-validate (max 2 cycles).
   - **If Debugger outputs `REPLANNED`:** Mark stage `blocked` in `STATE.md` and stop immediately.
8. **Complete:**
...
Unattended execution of all stages from start to finish.
...
2. **Execution Loop:**
   ...
   - Apply the **Unattended Addendum** (from `references/autonomy_policy.md`) to `stage-architect`, `implementer`, and `debugger`.
   - Transcribe all `Autonomous decisions` into `DECISIONS.md` immediately.
   - Keep validator strictly isolated with `NN-slug.md` only.
   - On stage validation PASS: Commit automatically: `git add -A && git commit -m "stage NN: <title>"`.
   - Immediately proceed to the next stage.
3. **Major Stop Conditions:**
   - Implementer stalls, stage-architect splits stage, debugger returns `REPLANNED`, or check/review/validation retry budget (2 cycles) is exhausted.
